...
class BrowserActionAPI {
  ...
  activate(extEvt, details) {
    const {
      eventType,
      extensionId,
      tabId
    } = details;
    debug(`activate [eventType: ${eventType}, extensionId: '${extensionId}', tabId: ${tabId}, senderId: ${extEvt.sender.id}]`);
  ...
  activateContextMenu(details, event) {
...
const resolveExtensionPath = (extension, uri) => {
  ...
  const resPath = path__WEBPACK_IMPORTED_MODULE_1__.join(extension.path, relpath); // prevent any parent traversals
...
class RuntimeAPI extends events__WEBPACK_IMPORTED_MODULE_0__.EventEmitter {
  constructor(ctx) {
    ...
    this.openOptionsPage = async event => {
...
/* harmony import */ var electron__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! electron */ "electron");
/* harmony import */ var electron__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(electron__WEBPACK_IMPORTED_MODULE_2__);
/* harmony import */ var _utils__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../utils */ "./src/browser/utils.ts");
/* harmony import */ var _windows__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./windows */ "./src/browser/api/windows.ts");
...
class TabsAPI {
  ...
  async create(event, details = {}) {
    let url = details.url; // TODO: add test for this process

    if (url && event.extension) {
      const urlInfo = (0,node_url__WEBPACK_IMPORTED_MODULE_1__.parse)(url || '');
      const baseURL = (0,_utils__WEBPACK_IMPORTED_MODULE_3__.ensureSuffix)(event.extension.url, '/');

      if (!urlInfo.protocol && !urlInfo.hostname) {
        url = node_path__WEBPACK_IMPORTED_MODULE_0___default().posix.join(baseURL, url);
  ...
  async query(event, info = {}) {
    ...
    const filteredTabs = Array.from(this.ctx.store.tabs).map(this.getTabDetails.bind(this)).filter(tab => {
      ...
      if (isSet(info.currentWindow)) {
        ...
        if (info.currentWindow && (currentWin === null || currentWin === void 0 ? void 0 : currentWin.id) !== tab.windowId) return false;
      }

      if (isSet(info.lastFocusedWindow)) {
        if (info.lastFocusedWindow && !lastFocusedWindow) return false;
        if (info.lastFocusedWindow && (lastFocusedWindow === null || lastFocusedWindow === void 0 ? void 0 : lastFocusedWindow.id) !== tab.windowId) return false;
      }
      ...
      if (isSet(info.windowId)) {
        if (info.windowId === TabsAPI.WINDOW_ID_CURRENT) {
          if (!currentWin || (currentWin === null || currentWin === void 0 ? void 0 : currentWin.id) !== tab.windowId) return false;
  ...
  onRemoved(tabId) {
    ...
    const windowId = details ? details.windowId : _windows__WEBPACK_IMPORTED_MODULE_4__.WindowsAPI.WINDOW_ID_NONE;
    const win = typeof windowId !== 'undefined' && windowId > -1 ? electron__WEBPACK_IMPORTED_MODULE_2__.BrowserWindow.getAllWindows().find(win => win.id === windowId) : null;
...
/* harmony import */ var _utils__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../utils */ "./src/browser/utils.ts");
...
class WindowsAPI {
  ...
  async getWindowFromId(event, id) {
    if (id === WindowsAPI.WINDOW_ID_CURRENT) {
      return (await this.ctx.store.windowsGetCurrent(event)) || this.ctx.store.getLastFocusedWindow();
    } else {
      return this.ctx.store.getWindowById(event, id);
    }
  }

  async get(event, windowId) {
    let win;
  ...
  async create(event, details) {
    const baseURL = (0,_utils__WEBPACK_IMPORTED_MODULE_1__.ensureSuffix)(event.extension.url, '/');
    let url = Array.isArray(details.url) ? details.url[0] : details.url || '';
    const urlInfo = (0,node_url__WEBPACK_IMPORTED_MODULE_0__.parse)(url);

    if (!urlInfo.protocol && !urlInfo.hostname) {
      url = baseURL + (0,_utils__WEBPACK_IMPORTED_MODULE_1__.unPrefix)(url, ['./', '/']);
    } else if (!urlInfo.hostname) {
      url = baseURL + (0,_utils__WEBPACK_IMPORTED_MODULE_1__.unPrefix)(url, ['./', '/']);
    }
  ...
  async update(event, windowId, updateProperties = {}) {
    ...
    if (props.state) {
      switch (props.state) {
        ...
        case 'fullscreen':
          if (win.fullScreenable) win.setFullScreen(true);else {
...
class ElectronChromeExtensions extends events__WEBPACK_IMPORTED_MODULE_1__.EventEmitter {
  ...
  async prependPreload() {
    ...
    const preloadPath = this.preloadPath || path__WEBPACK_IMPORTED_MODULE_2___default().join(this.modulePath, 'dist/preload.js');
...
/* harmony import */ var _isomorphic_ipc__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ../isomorphic/ipc */ "./src/isomorphic/ipc.ts");
...
(debug__WEBPACK_IMPORTED_MODULE_1___default().formatters.r) = value => {
  return value ? JSON.stringify(value, shortenValues, '  ') : value;
};

const debug = debug__WEBPACK_IMPORTED_MODULE_1___default()('electron-chrome-extensions:router');
...
class ExtensionRouter {
  ...
  sendEvent(extensionId, eventName, ...args) {
    ...
    for (const {
      ...
      host.send((0,_isomorphic_ipc__WEBPACK_IMPORTED_MODULE_2__.formatIpcName)(eventName), ...args);
    }
  }
...
class ExtensionStore extends events__WEBPACK_IMPORTED_MODULE_1__.EventEmitter {
  ...
  async getWindowById(event, windowId) {
    var _this$impl$getWindowB, _this$impl;

    const foundWindow = this.findWindowById(windowId);
    const window = (_this$impl$getWindowB = (_this$impl = this.impl).getWindowById) === null || _this$impl$getWindowB === void 0 ? void 0 : _this$impl$getWindowB.call(_this$impl, {
      event,
      foundWindow
    }, windowId);
  ...
  async windowsGetCurrent(event) {
    const wc = event.sender;
    let win = this.tabToWindow.get(wc) || electron__WEBPACK_IMPORTED_MODULE_0__.BrowserWindow.fromWebContents(wc);
  ...
  removeTab(tab) {
    ...
    (async () => {
      var _this$impl$getTabbedB, _this$impl2;

      const behaviors = await ((_this$impl$getTabbedB = (_this$impl2 = this.impl).getTabbedBrowserWindowBehavior) === null || _this$impl$getTabbedB === void 0 ? void 0 : _this$impl$getTabbedB.call(_this$impl2, {
        tabs: this.tabs,
        window: win
      ...
      if (!windowHasTabs && !(behaviors !== null && behaviors !== void 0 && behaviors.keepRefWindowOnAllTabsClosed)) {
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "ensureSuffix": () => (/* binding */ ensureSuffix),
/* harmony export */   "unPrefix": () => (/* binding */ unPrefix)
/* harmony export */ });
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "formatIpcName": () => (/* binding */ formatIpcName)
/* harmony export */ });
